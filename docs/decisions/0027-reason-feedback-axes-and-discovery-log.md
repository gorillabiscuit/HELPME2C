...
> **Amendment (2026-06-13, same day):** The original draft scoped *all* free text as discovery-only — "never a trusted preference signal." Reviewing a concrete case — a user typing *"I detest horror shows"* — showed that was too broad a brush. A direct preference **declaration** is a named aspect + polarity (a genre veto), not a confabulated **reason**, and by this ADR's own "harvest the named *what*" logic it can and should influence recommendations. The correction: free text reaches scoring **only through an extract → confirm → apply loop** — an LLM proposes the veto/preference, the user confirms it with one tap, and only the confirmed signal updates the model. Confabulated reasons ("the pacing felt off") stay discovery-only, and raw text is **never applied silently**. The sections below reflect this. Slice 1 only *logs* the text; the extract → confirm → apply loop is a later slice.
...
2. **Optional free-text capture, gated behind "None of these fit."** Tapping "None of these fit" (in either like or dislike mode) reveals an **optional** text box. It is never required, never gates progression, and submitting it empty is a valid completion. Raw free text is **never applied to scoring silently.** Two things come out of it: a **confabulated reason** ("the pacing felt off") feeds only **(a) taxonomy-discovery radar** and **(b) state-vs-content-vs-friction routing**, plus **(c)** the voice/agency of being heard; a **direct preference declaration** ("I detest horror") is a named aspect + polarity that reaches scoring **via extract → confirm → apply** — an LLM proposes the veto, the user confirms it with one tap, and only then does it update the model. We trust the named *what* (once confirmed), never the introspected *why*.
...
- **Raw free text fed *directly and silently* into scoring / a required field** — trusting unconfirmed prose reintroduces the Wilson & Schooler confabulation problem and the YouTube folk-theory betrayal (a silent veto the user can't see or correct; Mozilla 2022). PrefEval (ICLR 2025) shows the LLM-rec field quietly *assumes the stated preference is valid*; we don't. This rejects *direct, silent* use — **not** the extract → confirm → apply path, where the user confirms an extracted preference before it scores.
...
**The (a)/(b) split is the whole rationale for how free text is used.** There were always two distinct objections to free-text "tell us why": (a) the *mapping* problem — converting prose to system features was hard — and (b) the *introspection-validity* problem — the stated reason is confabulated and predicts future taste worse than the affective act (Wilson & Schooler 1991; sharpened by choice-blindness, Johansson et al. 2006). Real-time LLM analysis **dissolves (a) but does nothing for (b)**: an LLM that flawlessly structures a confabulated "why" yields high-fidelity confabulation. So a free-text *reason* is never trusted directly as a preference signal. But the (b) critique targets the introspected *why*, not a direct preference **declaration**: "I detest horror" is a named aspect + polarity (a genre veto) which — like the format-veto chip — is high-value and may reach scoring **through extract → confirm → apply** (LLM proposes the veto, user confirms, only then does it score; the confirmation step also fixes the folk-theory betrayal, because the user sees the signal land). Two further uses *survive* (b) with no confirmation because they never touch scoring — only the *what* to be named: **taxonomy discovery** (the text names a dimension our facets lack — e.g. "the fights," "it's trash reality TV") and **routing** (classify content-dislike vs wrong-mood vs friction). GATE (Li et al., ICLR 2025) corroborates that open elicitation "surfaces considerations users did not anticipate." Our full research read lives in [`preference-elicitation-research/`](../../preference-elicitation-research/report.md).
...
- **UX.** "None of these fit" → reveal an optional `<textarea>` with a "Skip" and a "Done" affordance; empty submit closes cleanly. Never blocks the *N of N* flow. Mirror the existing mood escape-hatch pattern (no slugs). The skip option is appended **server-side** with a structural `isNoneOfThese` flag so the client detects it by flag, never by matching the (LLM-generated) label text.
- **Free text → scoring (extract → confirm → apply) — later slice.** An LLM proposes candidate preference declarations (e.g. "avoid horror") from logged free text; the user confirms with one tap before any scoring effect; only confirmed signals update the preference model. Unconfirmed text and confabulated reasons stay discovery-only. Slice 1 only logs the text. The confirmation step is also what fixes the folk-theory betrayal — the user sees the signal land.
...
- A clean head-to-head showing *unconfirmed* LLM-extracted free-text signal predicts forward satisfaction as well as the confirmed path → drop the confirmation step and apply extractions directly. (Confirmed extractions already feed scoring per the amendment; the open question is whether confirmation is necessary.)
