...
> **Note:** This runbook references scripts that live on the `feat/v4-content-descriptors` branch and aren't on `main` yet. Once that PR merges, the scripts will be available from `main`. Until then, check out the branch to use them.
...
| `benchmark-rec-performance.ts` | p95 measurement of recompute + read against PROJECT.md §55 (<500ms p95). | $0 | ~10s |
...
| `catalog-broaden-prestige.ts` | Pull prestige TV from TMDB to fill catalog gaps (Breaking Bad, Sopranos, etc). Guarded by `WIDEN_OK=yes`. | $0 (TMDB free) | ~3 min |
