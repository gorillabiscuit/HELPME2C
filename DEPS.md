...
- **@dnd-kit/core** *(^6.1.0)* — drag-and-drop primitives. Powers the `/library?view=ranked` drag-to-reorder list. Per CLAUDE.md §3 ban on reinventing well-solved interaction patterns; replaced a hand-rolled HTML5-drag implementation that lacked drop indicators, had ghost-card duplication bugs, and was missing keyboard accessibility.
- **@dnd-kit/sortable** *(^8.0.0)* — sortable-list extensions built on @dnd-kit/core. Provides the `SortableContext` + `useSortable` primitives that match the rank-list use case.
- **@dnd-kit/utilities** *(^3.2.2)* — peer of @dnd-kit/sortable; supplies the `CSS.Transform.toString` helper used to apply the live drag transform to sortable items.
...
- **svix** *(^1.93.0)* — Clerk's webhook signature library; verifies `svix-id` / `svix-timestamp` / `svix-signature` headers against the `CLERK_WEBHOOK_SECRET`. Sole purpose is auth on the Clerk webhook handler at `/api/webhook/clerk`. Tiny (~3KB) and dependency-light.
