...
export const reasonFeedbackEvents = pgTable(
  'reason_feedback_events',
  {
  ...
  // Both the account-export read and the deletion cascade filter by user_id
  // on this append-only (unbounded) table — index it so neither seq-scans.
  (t) => [index('reason_feedback_events_user_id_idx').on(t.userId)],
);
