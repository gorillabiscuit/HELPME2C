...
- **Cross-package import direction.** `apps/web` imports from `packages/{shared,ui,ml}`. The mobile portion below is **provisional** until a real Expo / RN consumer exists (see [`apps/mobile/README.md`](apps/mobile/README.md) for why): `apps/mobile` is *intended* to import from `packages/{shared,ml}` and a separate `packages/mobile-ui` (NEVER `packages/ui` — web React ≠ React Native), but this boundary will be re-validated when Phase 2 opens. No app imports from another app. (If `apps/api` is extracted later per [ADR-0015](docs/decisions/0015-api-deployment-shape.md), it imports from `packages/{shared,ml}`.)
